...
# levels can be only numbers: 0,1,2,3 buttons can be
 # strings: '0','1','2','3' possible return values are numbers: -3,-2,-1,0,1,2,3
...
# If the elevator is on the 3rd level and the user touches button
 # '0' the elevator must go 3 levels down, so our function must return -3.
...
# We cannot endanger the lives of our passengers,
 # so if we get erronous inputs, our elevator must remain on the same level. So for example:
...
def goto(level, button)
	x = (-3..3).to_a
	if level.class == Fixnum && button.class == String && x.include?(level) && x.include?(button.to_i)
	  puts (button.to_i - level.to_i).to_i
	else
		puts 0
	end
end
